...
namespace WndApp2.DAL
{
    class KitapDAL
    {
        ...
        public Kitap KitapBul(int id)
        {
            ...
            SqlDataReader dr = cmd.ExecuteReader();
            dr.Read(); //Pk e göre arama yapıyorsak ya bir tane kayıt çıkar yada hiç çıkmaz.O yüzden while in içine yazmadık.
            Kitap kitap = new Kitap();
            kitap.KitapID = -1;
            if (dr.HasRows) //Geriye değer dönüyorsa doğru bulamıyorsa yanlıştır.(HasRoes)
            {
                kitap.KitapID = Convert.ToInt32(dr[0]);
                kitap.KitapAdi = dr[1].ToString();
                kitap.KategoriID = dr.IsDBNull(2) ? 0 : (int?)dr[2];
